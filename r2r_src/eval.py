...
class Evaluation(object):
    ...
    def __init__(self, splits, scans, tok):
        ...
        for split in splits:
            for item in load_datasets([split]):
                ...
                if args.dataset == 'RXR':
                    self.gt[item['instruction_id']] = item
    ...
    def _score_item(self, instr_id, path):
        ...
        if args.dataset in ['R4R', 'R6R', 'R8R', 'RXR']:
            sr = nav_error < self.error_margin
            self.scores['cls'].append(self.cls(gt['scan'], path, gt['path']))
            ndtw = self.ndtw(gt['scan'], path, gt['path'])
            sdtw = ndtw * sr
            self.scores['ndtw'].append(ndtw)
            self.scores['sdtw'].append(sdtw)
    ...
    def score(self, output_file):
        ...
        if args.dataset in ['R4R', 'R6R', 'R8R', 'RXR']:
            score_summary['cls'] = np.average(self.scores['cls'])
            score_summary['ndtw'] = np.average(self.scores['ndtw'])
            score_summary['sdtw'] = np.average(self.scores['sdtw'])
    ...
    def length(self, scan, nodes):
        if isinstance(nodes[0], str):
            ...
            return float(np.sum([self.distances[scan][edge[0][0]][edge[1][0]]
                                 for edge in zip(nodes[:-1], nodes[1:])]))
    ...
    def cls(self, scan, prediction, reference):
        coverage = np.mean([np.exp(
            -np.min([self.distances[scan][u][v[0]] for v in prediction]) / self.error_margin
        ) for u in reference])
