...
class R2RBatch():
    ...
    def __init__(self, feature_store, batch_size=100, seed=10, splits=['train'], tokenizer=None,
        ...
        #     self.aug_path_db = h5py.File(os.path.join(args.data_dir, args.aug_path), 'r')

        scans = []
        for split in splits:
            # for i_item, item in enumerate(random.sample(load_datasets([split]), 60)):
            for i_item, item in enumerate(load_datasets([split])):
                ...
                if "/" in split:
                    ...
                    if 'new_instructions' in new_item:
                        new_item['new_instr_enc'] = new_item['new_instructions'][0]
                    if 'aug_new_instructions' in item:
                        if args.multi_aug_instr:
                            new_item['aug_new_instr_enc'] = new_item['aug_new_instructions']
                        else:
                            new_item['aug_new_instr_enc'] = new_item['aug_new_instructions'][0]
                    ...
                    if 'train' in split and 'val' not in split:
                        instructions = item['instructions'][:3]
                    else:
                    ...
                    for j, instr in enumerate(instructions):
                        ...
                        if 'new_instructions' in new_item:
                            if len(new_item['new_instructions'][j][0]) != args.maxInput:
                                new_instructions = []
                                for new_instr in new_item['new_instructions'][j]:
                                    new_instructions.append(new_instr + [0 for index in range(args.maxInput - len(new_item['new_instructions'][j][0]))])
                            else:
                                new_instructions = new_item['new_instructions'][j]
                            new_item['new_instr_enc'] = new_instructions.copy()
                        if 'aug_new_instructions' in item:
                            if not args.multi_aug_instr:
                                if len(new_item['aug_new_instructions'][j][0]) != args.maxInput:
                                    new_instructions = []
                                    for new_instr in new_item['aug_new_instructions'][j]:
                                        new_instructions.append(new_instr + [0 for index in range(args.maxInput - len(new_item['aug_new_instructions'][j][0]))])
                                else:
                                    new_instructions = new_item['aug_new_instructions'][j]
                            else:
                                new_instructions = new_item['aug_new_instructions']
                            new_item['aug_new_instr_enc'] = new_instructions.copy()
                        if new_item['instr_encoding'] is not None:  # Filter the wrong data
                            self.data.append(new_item)
                            scans.append(item['scan'])
                        # except:
    ...
    def _get_obs(self):
        ...
        for i, (feature, state, second) in enumerate(self.env.getStates()):
            ...
            if 'aug_paths' in item:
                obs[-1]['aug_paths'] = item['aug_paths']
            if 'new_instr_enc' in item:
                obs[-1]['new_instr_enc'] = item['new_instr_enc']
            if 'aug_new_instr_enc' in item:
                obs[-1]['aug_new_instr_enc'] = item['aug_new_instr_enc']
