...
class VLNBERT(nn.Module):
    def __init__(self, feature_size=2048+128):
        ...
        if args.aug_lang is not None:
            self.lang_linear = nn.Linear(args.maxInput, 1)
            self.lang_proj_layer = nn.Sequential(nn.Linear(hidden_size, hidden_size // 2),
                                                  nn.BatchNorm1d(hidden_size // 2), nn.ReLU(),
                                                  nn.Linear(hidden_size // 2, args.multi_feat_dim),
                                                  nn.BatchNorm1d(args.multi_feat_dim), nn.ReLU(),
                                                  nn.Linear(args.multi_feat_dim, args.multi_feat_dim),
                                                  nn.BatchNorm1d(args.multi_feat_dim))
            self.lang_pred_layer = nn.Sequential(nn.Linear(args.multi_feat_dim, args.multi_feat_dim),
                                                  nn.BatchNorm1d(args.multi_feat_dim), nn.ReLU(),
                                                  nn.Linear(args.multi_feat_dim, args.multi_feat_dim))
        if args.aug_path is not None:
            self.vision_proj_layer = nn.Sequential(nn.Linear(hidden_size, hidden_size // 2),
                                                  nn.BatchNorm1d(hidden_size // 2), nn.ReLU(),
                                                  nn.Linear(hidden_size // 2, args.multi_feat_dim),
                                                  nn.BatchNorm1d(args.multi_feat_dim), nn.ReLU(),
                                                  nn.Linear(args.multi_feat_dim, args.multi_feat_dim),
                                                  nn.BatchNorm1d(args.multi_feat_dim))
            self.vision_pred_layer = nn.Sequential(nn.Linear(args.multi_feat_dim, args.multi_feat_dim),
                                                  nn.BatchNorm1d(args.multi_feat_dim), nn.ReLU(),
                                                  nn.Linear(args.multi_feat_dim, args.multi_feat_dim))
