...
def load_datasets(splits):
    ...
    for split in splits:
        ...
        if "/" not in split:
            with open(os.path.join(args.data_dir, 'data/R2R_%s.json' % split)) as f:
                new_data = json.load(f)

            if args.aug_val and 'val_unseen' in split:
                if int(args.aug_val_type.split('_')[-1]) >= 10:
                    file_name = args.aug_val_type[:-3]
                else:
                    file_name = args.aug_val_type[:-2]
                with open(os.path.join(args.data_dir, 'data/%s/R2R_%s.json' % (file_name, split + '_%s'%args.aug_val_type))) as f:
                    aug_val_data = json.load(f)
                aug_ind = 0
                for ind in range(len(new_data)):
                    new_instrs = []
                    for instr in new_data[ind]['instructions']:
                        new_instrs.append(aug_val_data[aug_ind])
                    new_data[ind]['instructions'] = new_instrs.copy()
                print('Substitute val unseen data by augment data {}'.format(args.aug_val_type))
        else:
            print('\nLoading prevalent data for pretraining...')
            with open(os.path.join(args.data_dir, split)) as f:
                new_data = json.load(f)

        if 'train' in split and 'val' not in split:
            new_data = new_data[:int(len(new_data)*args.data_percent)]
        data_len = len(new_data)

        if '/' in split and args.aug_path is not None:
            aug_path1 = args.aug_path
            aug_path2 = args.aug_path2
        elif 'train' in split and 'val' not in split and args.aug_train_path is not None:
            aug_path1 = args.aug_train_path
            aug_path2 = args.aug_train_path2
        else:
            aug_path1 = None
            aug_path2 = None

        if aug_path1 is not None:
            new_new_data = []
            aug_f1 = open(os.path.join(args.data_dir, aug_path1), 'r')
            aug_f2 = open(os.path.join(args.data_dir, aug_path2), 'r')
            aug_objects1 = ijson.items(aug_f1, 'item')
            aug_objects2 = ijson.items(aug_f2, 'item')
            ind = 0
            invalid = 0
            for obj1, obj2 in zip(aug_objects1, aug_objects2):
                new_new_data_i = new_data[ind]
                ind += 1

                if args.aug_path_type == 'distance':
                    pos_dis = new_new_data_i['distance'] * args.pos_thr
                    neg_dis = new_new_data_i['distance'] * args.neg_thr
                elif args.aug_path_type == 'len':
                    pos_len = int(len(new_new_data_i['path']) * args.pos_thr)
                    neg_len = int(len(new_new_data_i['path']) * args.neg_thr)
                pos_aug_paths = []
                neg_aug_paths = []
                for obj_ind, obj in enumerate([obj1, obj2]):
                    for aug_ind, aug_path in enumerate(obj):
                        if args.aug_path_type == 'ndtw':
                            if aug_path['ndtw'] >= args.pos_thr:
                                pos_aug_paths.append(aug_path['path'])
                            elif aug_path['ndtw'] <= args.neg_thr:
                                neg_aug_paths.append(aug_path['path'])
                        elif args.aug_path_type == 'distance':
                            if aug_path['distance'] <= pos_dis:
                                pos_aug_paths.append(aug_path['path'])
                            elif aug_path['distance'] >= neg_dis:
                                neg_aug_paths.append(aug_path['path'])
                        elif args.aug_path_type == 'len':
                            if len(aug_path['path']) <= pos_len:
                                pos_aug_paths.append(aug_path['path'])
                            elif len(aug_path['path']) >= neg_len:
                                neg_aug_paths.append(aug_path['path'])

                if len(pos_aug_paths) == 0 or len(neg_aug_paths) == 0:
                    invalid += 1
                new_new_data_i['aug_paths'] = {}
                new_new_data_i['aug_paths']['ind'] = ind - 1
                new_new_data_i['aug_paths']['pos'] = pos_aug_paths.copy()
                new_new_data_i['aug_paths']['neg'] = neg_aug_paths.copy()
                new_new_data.append(new_new_data_i)
                if ind >= data_len:
                    break
            aug_f1.close()
            aug_f2.close()
            print('origin data len: {}'.format(len(new_data)))
            print('new data len: {}'.format(len(new_new_data)))
            print('invalid data filtered by pos_thr and neg_thr: {}'.format(invalid))
            new_data = new_new_data

        # augmented instructions
        if '/' in split and args.aug_lang is not None:
            aug_lang = args.aug_lang
            sub_lang = split.split('.')[0] + '_tok.json'
        elif 'train' in split and 'val' not in split and args.aug_train_lang is not None:
            aug_lang = args.aug_train_lang
            sub_lang = 'data/R2R_%s_tok.json' % split
        else:
            aug_lang = None
            sub_lang = None

        if aug_lang is not None:
            new_new_data = []
            ind = 0
            tok_file = aug_lang.split('.')[0] + '_tok.json'
            with open(os.path.join(args.data_dir, tok_file), 'r') as f:
                aug_data_tok = json.load(f)
            if sub_lang is not None:
                with open(os.path.join(args.data_dir, sub_lang), 'r') as f:
                    sub_data_tok = json.load(f)

            if sub_lang is not None:
                for obj_ind, obj in enumerate(new_data):
                    new_new_data_i = obj.copy()
                    new_instructions = new_new_data_i['new_instructions']
                    if not isinstance(new_instructions, list):
                        new_instructions = ast.literal_eval(new_instructions)
                    new_new_data_i['new_instructions'] = sub_data_tok[obj_ind]

                    aug_new_instructions = []
                    for instrs in new_instructions:
                        aug_sub_instrs = []
                        for sub_instr in instrs:
                            aug_sub_instrs.append(aug_data_tok[ind])
                            ind += 1
                        aug_new_instructions.append(aug_sub_instrs)
                    new_new_data_i['aug_new_instructions'] = aug_new_instructions.copy()
                    new_new_data.append(new_new_data_i)
            else:
                for obj_ind, obj in enumerate(new_data):
                    new_new_data_i = obj.copy()
                    new_instructions = new_new_data_i['instructions']
                    aug_new_instructions = []
                    for instrs in new_instructions:
                        aug_new_instructions.append([aug_data_tok[ind]])
                        ind += 1
                    new_new_data_i['aug_new_instructions'] = aug_new_instructions.copy()
                    new_new_data.append(new_new_data_i)
                assert ind == len(aug_data_tok)
            assert ind == len(aug_data_tok) and len(sub_data_tok) == len(new_data)
            new_data = new_new_data
...
import torch
def length2mask(length, size=None, gpu_id=0):
    batch_size = len(length)
    size = int(max(length)) if size is None else size
    mask = (torch.arange(size, dtype=torch.int64).unsqueeze(0).repeat(batch_size, 1)
                > (torch.LongTensor(length) - 1).unsqueeze(1)).cuda(gpu_id)
    return mask
...
def viewpoint_drop_mask(viewpoint, seed=None, drop_func=None, gpu_id=0):
    local_seed = hash(viewpoint) ^ seed
    torch.random.manual_seed(local_seed)
    drop_mask = drop_func(torch.ones(2048).cuda(gpu_id))
    return drop_mask
